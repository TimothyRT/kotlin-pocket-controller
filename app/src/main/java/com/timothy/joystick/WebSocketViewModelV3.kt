 ...
 * 5. Shake
 */
class WebSocketViewModelV3 : ViewModel() {
    ...
    data class GestureEvent(
        ...
        val isCalibrationFeedback: Boolean = false  // True when detected during calibration
    )
    ...
    // Message channel - drops oldest messages if buffer is full (prevents crash from message buildup)
        ...
        viewModelScope.launch {
            try {
                ...
                if (wsSession.isActive) {
                    ...
                    startSending(wsSession)  // Start the sender coroutine
    ...
    /**
     * Send message via channel (non-blocking, drops old messages if overwhelmed)
     */
    fun send(content: String) {
        if (_connectionState.value != ConnectionState.Connected) return

        // Use trySend - non-blocking, drops if buffer full
        sendChannel.trySend(content)
    }

    /**
     * Single coroutine that processes all outgoing messages
     */
    ...
    private fun handleMessage(text: String) {
        try {
            ...
            Log.e(TAG, "Error handling message: $text, error: $e")
        }
    }
    ...
    private fun handleDetectionMessage(text: String) {
        ...
        if (parts.size >= 3) {
            ...
            _lastGesture.postValue(GestureEvent(name, confidence, isCalibrationFeedback = true))
        }
    }
    ...
    private fun handlePhaseMessage(text: String) {
        ...
        // Create instruction with clear action prompt
        val actionInstruction = when (phase) {
            CalibrationPhase.IDLE -> instruction
            else -> {
                if (current < total) {
                    "$instruction\n\n👉 Do it NOW! (${current + 1}/$total)"
                } else {
                    instruction
                }
            }
        ...
        _calibrationState.postValue(CalibrationState(
            phase = phase,
            instruction = actionInstruction.ifEmpty { phaseInstructions[phase] ?: "" },
    ...
    private fun handleGestureMessage(text: String) {
        ...
        if (parts.size >= 3) {
            val name = parts[1]
            val confidence = parts[2].toFloatOrNull() ?: 0f
            _lastGesture.postValue(GestureEvent(name, confidence, isCalibrationFeedback = false))
        }
    }
